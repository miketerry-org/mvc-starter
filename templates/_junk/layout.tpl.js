// layout.tpl.js: implements template literal for main layout html page

// load all required packages
const xss = require("xss");

// export  the template literal function for the main index web page
module.exports = (project, auth, content) => {
  return `
  <html lang="en">
    <head>
      <meta charset="UTF-8" />
      <meta name="viewport" content="width=device-width, initial-scale=1.0" />
      <title>${xss(project.title)}</title>
    ...
    <body class="bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      ...
      <header class="header py-6 px-4">
        ...
        >
          ...
          <div class="middle-block text-center md:text-center">
            <h1 class="text-3xl font-bold">${xss(project.title)}</h1>
            <h2 class="text-xl text-gray-600">${xss(project.slogan)}</h2>
          </div>
      ...
      <!-- Main Content -->
      <main class="main py-6 px-4">${xss(content)}</main>

      <!-- Footer -->
      <footer class="footer py-4 text-center text-sm text-gray-500">
        <p>© Copyright ${xss(project.year)} ${xss(project.owner)}</p>
      </footer>
    </body>
  </html>
  `;
